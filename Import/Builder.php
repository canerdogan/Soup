 ...
 * Soup_Import_Builder
 *
 * Import builder is responsible of building Soup_Record classes
 ...
 */
class Soup_Import_Builder extends Soup_Builder
{
    /**
     ...
     * @var string
     */
    protected $_baseClassPrefix = 'Base_';
     ...
     * @var string $suffix
     */
    protected $_baseClassesDirectory = 'Base';
     ...
     * @var string
     */
    protected $_baseClassName = 'Soup_Record';
    protected $_baseTableClassName = 'Soup_Table';
    ...
    {
//         $manager = Soup_Manager::getInstance();
        if ($tableClass = 'Soup_Table') {
    ...
    {
        $name = 'set' . Soup_Inflector::classify($key);
    ...
    /*
     * Build the table definition of a Soup_Record object
     *
     * @param  string $table
...
//         }
        ...
        $ret[$i] = $this->buildSetRelation($definition);
        $i++;
        ...
        return PHP_EOL . "    public function __construct()" . PHP_EOL . '    {' . PHP_EOL . '        ' . $code . PHP_EOL . '    }';
    }
     ...
     * @return string
     */
    public function buildSetRelation(array $definition)
    {
        $ret = array();
        $i = 0;
        ...
        if (isset($definition['relations']) && is_array($definition['relations']) && ! empty($definition['relations'])) {
            foreach ($definition['relations'] as $name => $relation) {
                ...
                if ( ! isset($relation['type'])) {
                    $relation['type'] = Soup_Table::RELATION_ONE;
                    
                }

                if ($relation['type'] === Soup_Table::RELATION_ONE) {
...
//         }
    ...
    public function buildColumns(array $columns)
    {
        ...
        foreach ($columns as $name => $column) {
            ...
            // Primary columns are implied to be notnull in Soup
            ...
            // Soup defaults to 0 if it is a primary key
    ...
    public function buildPhpDocs(array $definition)
    {
        ...
        $ret[] = '';
        $ret[] = 'This class has been auto-generated by the Soup ORM Framework';
        $ret[] = '';
...
//                     $name = $name;
            ...
            if (isset($definition['relations']) && ! empty($definition['relations'])) {
                foreach ($definition['relations'] as $relation) {
                    $type = (isset($relation['type']) && $relation['type'] == Soup_Table::RELATION_MANY) ? 'Soup_Collection' : $this->_classPrefix . $relation['class'];
...
//         $classname = $name;
//         if (class_exists("Soup_Template_$name", true)) {
//             $classname = "Soup_Template_$name";
//         }
...
//     }
    ...
    public function buildAttributes(array $attributes)
    {
        ...
        foreach ($attributes as $key => $value) {
            ...
            if (is_bool($value))
            {
              $values[] = $value ? 'true':'false';
                ...
                foreach ($value as $attr) {
                    $const = "Soup_Core::" . strtoupper($key) . "_" . strtoupper($attr);
                    if (defined($const)) {
                        $values[] = $const;
                    } else {
                        $values[] = "'" . $attr . "'";
            ...
            $build .= "        \$this->setAttribute(Soup_Core::ATTR_" . strtoupper($key) . ", " . $string . ");" . PHP_EOL;
        }
    ...
    public function buildDefinition(array $definition)
    {
        if ( ! isset($definition['className'])) {
            throw new Soup_Import_Builder_Exception('Missing class name.');
        }
        ...
        if (isset($definition['is_base_class']) && $definition['is_base_class']) {
        	$className = $this->_baseClassPrefix . $definition['className'];
        } else {
...
//             $setUpCode = $this->buildSetRelation($definition);
            $setUpCode = null;
        } else {
    ...
    {
        return Soup_Query::getTable('%s');
    }", $className, $definition['className']);

        $docBlock = array();
        $docBlock[] = $className;
        $docBlock[] = '';
        $docBlock[] = 'This class has been auto-generated by the Soup ORM Framework';
    ...
    public function writeTableClassDefinition(array $definition, $path, $options = array())
    {
        ...
        Soup_Helper::makeDirectories(dirname($writePath));

        Soup_Helper::loadModel($className, $writePath);
    ...
    public function writeDefinition(array $definition)
    {
        ...
        // If it is the base class of the Soup record definition
        ...
        if (isset($definition['connection']) && $definition['connection']) {
            ...
            $code .= "Soup_Manager::getInstance()->bindComponent('" . $definition['connectionClassName'] . "', '" . $definition['connection'] . "');" . PHP_EOL;
        }
        ...
        if (isset($bytes) && $bytes === false) {
            throw new Soup_Import_Builder_Exception("Couldn't write file " . $writePath);
        }

        Soup_Helper::loadModel($definition['className'], $writePath);
    }
}