 ...
 * Soup_Import_Schema
 *
 * Class for importing Soup_Record classes from a yaml schema definition
 *
 * @package     Soup
 * @subpackage  Import
...
class Soup_Import_Schema
{
     ...
     * This array is forwarded to Soup_Import_Builder
     *
     * @var array
     */
     ...
     * importSchema
     *
     * A method to import a Schema and translate it into a Soup_Record object
     *
     * @param  string $schema       The file containing the XML schema
     * @param  string $format       Format of the schema file
     * @param  string $directory    The directory where the Soup_Record class will be written
    ...
    public function importSchema($schema, $format = 'yml', $directory = null, $models = array())
    {
        ...
        if (count($array) == 0) { 
            throw new Soup_Import_Exception(
    ...
    public function parseSchema($schema, $type)
    {
        ...
        $array = Soup_Parser::load($schema, $type);
        ...
        foreach ($array as $className => $table) {
            $array[$className] = Soup_Lib::arrayDeepMerge($globals, $array[$className]);
        }
        ...
        foreach ($array as $className => $table) {
            ...
            if (isset($table['tableName']) && $table['tableName']) {
                ...
                if (isset($table['inheritance']['type']) && ($table['inheritance']['type'] == 'column_aggregation')) {
                    ...
                    $tableName = Soup_Inflector::tableize($className);
                }
            }
            ...
            $build[$className] = Soup_Lib::arrayDeepMerge($table, $build[$className]);
            ...
            if (isset($definition['inheritance']['extends']) && isset($definition['inheritance']['type']) && ($definition['inheritance']['type'] == 'simple' || $definition['inheritance']['type'] == 'column_aggregation')) {
                ...
                foreach ($moves as $move => $resetValue) {
                    if (isset($array[$parent][$move]) && isset($definition[$move])) {
                        $array[$parent][$move] = Soup_Lib::arrayDeepMerge($array[$parent][$move], $definition[$move]);
            ...
            foreach ($relations AS $alias => $relation) {
                ...
                if (isset($relation['refClass'])) {
                    ...
                    if (isset($relation['foreignType'])) {
                        ...
                        $newRelation['type'] = $relation['type'] === Soup_Relation::ONE ? Soup_Relation::MANY:Soup_Relation::ONE;
                    }
                }
