 ...
 * 
 */
class Soup_Inflector {
	
	/**
	 * Doctrine'den alındı :)
     * Model isimleri için çevirici 'ModelName' den 'model_name' e çevirir
     *
     * @param  string $word  kelime
     * @return string $word  tableize edilmiş kelimemiz
     */
    ...
    /**
     * Doctrine'den alındı :)
     * Tablo isimlerini model isimlerine çevirici 'tablo_adi' dan 'TabloAdi' na çevirir.
     *
     * @param string  $word  kelime
     * @return string $word  Classified edilmiş kelimemiz
     */
    public static function classify($word)
    {
        ...
        if (!isset($cache[$word])) {
            ...
            $classify = preg_replace_callback('~(_?)([-_])([\w])~', array("Soup_Inflector", "classifyCallback"), ucfirst(strtolower($word)));
            $cache[$word] = $classify;
        }
        return $cache[$word];
    }
	...
	/**
	 * Doctrine'den alındı :)
     * Kelimeleri birleştiriyor getiriyor
     *
	...
	/**
     * Herhangi bir string'i urlde kullanılabilir hale getirir. 'My first blog post' tan 'my-first-blog-post' a çevirir.
     *
...
//     	return '`' . $smt . '`';
    ...
    public static function fixIndexName($idx)
    {
    	$indexPattern   = '/^'.preg_replace('/%s/', '([a-z0-9_]+)', "%s_idx").'$/i';
    	$indexName      = preg_replace($indexPattern, '\\1', $idx);
    	if ($indexName && ! strcasecmp($idx, self::getIndexName($indexName))) {
    		return $indexName;
    	}
    	return $idx;
    }
    
    public static function getIndexName($idx)
    {
    	return sprintf("%s_idx",
