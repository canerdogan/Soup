 ...
 * @since       1.0
 */
class Soup_Import_Mysql extends Soup_Import
{
    protected $sql  = array(
    ...
    public function listTableConstraints($table)
    {
        ...
        if ($this->conn->getAttribute(Soup_Core::ATTR_FIELD_CASE) && ($this->conn->getAttribute(Soup_Core::ATTR_PORTABILITY) & Soup_Core::PORTABILITY_FIX_CASE)) {
            if ($this->conn->getAttribute(Soup_Core::ATTR_FIELD_CASE) == CASE_LOWER) {
